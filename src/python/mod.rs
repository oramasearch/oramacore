...
/// Initializes the Python environment for MCP service only.
/// Embeddings no longer use Python - they use the native Rust fastembed crate.
pub fn initialize_for_mcp() -> PyResult<()> {
    let python_scripts_dir = extract_python_scripts().map_err(|e| {
    ...
    Python::attach(|py| {
        ...
        initialize_python_env(py, &python_scripts_dir)?;
        set_global_logging_level(py)?;
...
fn initialize_python_env(py: Python<'_>, python_scripts_dir: &PathBuf) -> PyResult<()> {
    ...
    let venv_dir =
        std::env::var("ORAMACORE_PYTHON_VENV_DIR").unwrap_or_else(|_| DEFAULT_VENV_DIR.to_string());
...
fn extract_python_scripts() -> anyhow::Result<PathBuf> {
    ...
    let scripts_dir = temp_base.join(format!("oramacore_python_scripts_{}", std::process::id()));
