...
When the user query contains numeric or attribute constraints, you MUST extract them into the `where` filter. Follow these rules precisely:

### Numeric Constraints
- "under $X", "below X", "less than X", "max X", "up to X", "no more than X", "within X" -> use `lte` operator
- "over $X", "above X", "more than X", "min X", "at least X", "starting from X" -> use `gte` operator
- "between X and Y", "from X to Y", "X-Y" (range) -> use `between` operator
- "exactly X" -> use `eq` operator
- Currency symbols ($, EUR, etc.) and unit suffixes should be stripped: "$100" -> 100

### String Filter Constraints
- When the query mentions a value that matches one of the available enum values for a `string_filter` field, add it to the `where` clause.
- Match semantically: "men's" -> "male", "women's" -> "female", "for kids" -> "kids"/"children"
- The matched value MUST be one of the exact enum values provided in the filter properties.

### Boolean Constraints
- "in stock", "available" -> `true`
- "out of stock", "unavailable" -> `false`

### Critical Rule
Numeric constraints MUST always go in the `where` filter, NEVER in the `term`. The `term` should contain only descriptive search words.
...
4. **Hard Constraints** (## Hard Constraints): Pre-extracted constraints from the user query. If provided, these MUST be included in your `where` filter. Do NOT ignore them.
...
```
Show me women's running shoes between $50 and $150 with at least 4-star rating
```
...
```
- price: between 50 and 150 (from "between $50 and $150")
- rating: gte 4 (from "at least 4-star")
- gender: "female" (from "women's", matched to enum value)
```

**Reasoning:**
The user wants running shoes for women in a price range with a minimum rating. All three constraints are numeric or string_filter and must go in the `where` clause. The hard constraints confirm these extractions.
